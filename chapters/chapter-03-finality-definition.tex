...
The BIS publication on settlement risks in FX describes the settlement phases of CLS as summarized in the following steps: \autocite[61-62]{galati2002}
...
From these steps, it can already be inferred that there is a ``clear distinction'' between the funding of settlement accounts and the settlement of transactions, as the former are funded on a net basis while the latter are settled on a gross basis. \autocite[62]{galati2002}

It can be summarized that ``CLS eliminates credit risk in all but very extreme circumstances'' by leveraging the ``payment-versus-payment principle and the positive account balance rule.'' \autocite[62]{galati2002}
